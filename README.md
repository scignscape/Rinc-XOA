...
}$DogLeash}$ Research Objects are assumed to be $\color{#a65450}\text{C++}$ code libraries that compile to a single recognized 
...
Assuming a Research Application is primarily implemented in $\color{#a65450}\text{C++}$, users should have the option of 
extending or fine-tuning this code without writing their own $\color{#a65450}\text{C++}$ files that would need a separate 
...
The first issue is exposing $\color{#a65450}\text{C++}$ procedures/methods to the script runtime.  The point here is that 
