 ...
 //?       _zipCompleter->setStateProvinceComboBox(_stateCombo);
    ...
    void populateLayout()
    {
        if (_typeOfLayout==ZipCodesWidget::FormLayout) {
            ...
            _form->addRow(_countryLabel, cb_country_);
...
//?            _grid->addWidget(_country, row, 1, 1, 1);
            _grid->addWidget(cb_country_, row, 1, 1, 1);
        }
    }
...
//?        QWidget::setTabOrder(_zip, _country);
        QWidget::setTabOrder(_zip, cb_country_);
...
void ZipCodesWidget::addMapping(QDataWidgetMapper *mapper, const int section, Mapping mapping) const
{
    ...
    case CountryIso: mapper->addMapping(d->cb_country_, section, "currentIsoCountry"); break;
    case CountryLocale: mapper->addMapping(d->cb_country_, section, "currentCountry"); break;
//?
