        ...
        m_useCheckBox(false)
{
   ...
   //?      m_grid->addWidget(m_detailsButton, 0, 2);
    m_grid->addWidget(btn_details_, 0, 2);
...
//?        topHeight = qMax(m_detailsButton->height(), topHeight);
      topHeight = qMax(btn_details_->height(), topHeight);
...
void DetailsWidgetPrivate::updateControls()
{
    ...
    btn_details_->setChecked(m_state == DetailsWidget::Expanded && m_widget);
//?    m_detailsButton->setChecked(m_state == DetailsWidget::Expanded && m_widget);
    btn_details_->setVisible(m_state == DetailsWidget::Expanded || m_state == DetailsWidget::Collapsed);
...
void DetailsWidget::enterEvent(Q_Enter_Event * event)
{
